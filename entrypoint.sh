...
if [ -f /app/nginx.conf ]; then
    ...
    content_server=$content_server'    location /stub_status {\n'
    content_server=$content_server'        stub_status;\n'
    ...
    content_server=$content_server"    listen ${USE_SSL_PORT} ssl http2 default_server;\n"
    content_server=$content_server"    ssl_certificate /etc/ssl/certs/nginx-selfsigned.crt;\n"
    content_server=$content_server"    ssl_certificate_key /etc/ssl/private/nginx-selfsigned.key;\n"
    content_server=$content_server'    location / {\n'
    content_server=$content_server'        include uwsgi_params;\n'
    content_server=$content_server'        uwsgi_pass unix:///tmp/uwsgi.sock;\n'
    content_server=$content_server'    }\n'
